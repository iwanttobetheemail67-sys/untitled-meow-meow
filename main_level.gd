...
var null_object = preload("res://null_object.tscn")
...
func _ready():
	...
	$Player/Camera2D/scaling_spirt/eyes.visible = false
	...
	if AudioHandler.spawn_cheat == true:
		spawn_timer.wait_time = AudioHandler.cheat_spawn
	...
	if AudioHandler.super_hard_mode == true:
		$Player/Camera2D/scaling_spirt/eyes.visible = true
...
func _process(_delta: float) -> void:
	...
	score_text.text = str(int(AudioHandler.score))
...
func _on_timer_timeout() -> void:
	if AudioHandler.score_freeze == false:
		AudioHandler.score = AudioHandler.score + (challenge_multipler + time_mult) * 100
	...
	if AudioHandler.difficulty >= 25000:
		if AudioHandler.diff_cheat == false:
			AudioHandler.difficulty = 25000
...
func _on_spawn_timer_timeout() -> void:
	if AudioHandler.fish_mode == true:
		return
	for i in range(AudioHandler.spawn_rate):
		spawn_seed = randi_range(1, 100)
		evil_seed = randi_range(1, 10)
		x_pos = randi_range(-6000, 6000)
		y_pos = randi_range(-3000, -2000)
		if spawn_seed <= 10:
			if AudioHandler.evil_drops == true:
				if evil_seed <= evil_chance:
					object = anger_goober.instantiate()
				else:
					object = fall_health.instantiate()
			else:
				object = fall_health.instantiate()
		elif spawn_seed >= 90:
			if AudioHandler.evil_drops == true:
				if evil_seed <= evil_chance:
					object = fall_slow.instantiate()
				else:
					object = falling_speed.instantiate()
			else:
				object = falling_speed.instantiate()
		elif spawn_seed < 90 and spawn_seed >= 80:
			if AudioHandler.evil_drops == true:
				if evil_seed <= evil_chance:
					object = frozen_fish.instantiate()
				else:
					object = fish.instantiate()
			else:
				object = fish.instantiate()
		elif spawn_seed == 50:
			if AudioHandler.evil_drops == true:
				if evil_seed <= evil_chance:
					if AudioHandler.no_death_skulls == false:
						object = death_skull.instantiate()
					else:
						object = angel.instantiate()
				else:
					object = angel.instantiate()
			else:
				object = angel.instantiate()
			...
			if AudioHandler.evil_drops == true:
				if evil_seed <= evil_chance:
					object = poison.instantiate()
				else:
					object = gold.instantiate()
			else:
				object = gold.instantiate()
		elif spawn_seed == 67:
			if AudioHandler.evil_drops == true:
				object = upside_down.instantiate()
			else:
				object = null_object.instantiate()
		else:
				object = goober.instantiate()
		get_parent().add_child.call_deferred(object)
		if is_instance_valid(object):
			object.position = Vector2(x_pos, y_pos)
	spawn_timer.wait_time = spawn_timer.wait_time - 0.0005
	if spawn_timer.wait_time < 0.01:
